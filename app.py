...
def enviar_correo_verificacion(email_destino, codigo):
    try:
        ...
        <body style="font-family: sans-serif; background-color: #f4f4f5; padding: 20px;">
            <div style="max-width: 500px; margin: auto; background: white; padding: 30px; border-radius: 12px; border: 1px solid #e4e4e7;">
                <h2 style="color: #4f46e5; text-align: center;">Verificación de Identidad</h2>
                <p>Se ha solicitado la creación de una cuenta en el Sistema de Gestión de Inventarios.</p>
                <p>Por seguridad empresarial, introduce el siguiente código de activación de un solo uso:</p>
                <div style="background-color: #f1f5f9; padding: 15px; text-align: center; font-size: 24px; font-weight: bold; letter-spacing: 4px; color: #1e293b; margin: 20px 0; border-radius: 8px;">
                    {codigo}
                </div>
            </div>
        </body>
        ...
        msg['Subject'] = f"{codigo} es tu código de verificación de seguridad"
        ...
        print(f"\n[SISTEMA LOCAL] Token generado: {codigo} para {email_destino} (Aviso SMTP: {e})\n")
        return False

# ==========================================
# ESTRUCTURA DE DB Y AUTO-MIGRACIÓN
# ==========================================
def init_db():
...
#     df = pd.read_csv(CSV_FILENAME)
        ...
        if df is not None:
            try:
                ...
                df['Nombre_Equipo'] = df['MARCA'].astype(str).str.upper().str.strip() + " " + \
                                      df['MODELO'].astype(str).str.strip() + " | " + \
                                      df['Procesador'].astype(str).str.strip() + " | " + \
                                      df['Memoria'].astype(str).str.strip()
                
                stock_counts = df.groupby(['Nombre_Equipo', 'MARCA']).size().reset_index(name='stock')
                
                equipos_procesados = []
                for _, row in stock_counts.iterrows():
                    nombre = row['Nombre_Equipo']
                    marca = str(row['MARCA']).upper().strip()
                    stock = int(row['stock'])
                    
                    precio_semanal_base = 45.00
                    nombre_upper = nombre.upper()
                    
                    if "I9" in nombre_upper or "XEON" in nombre_upper or "VICTUS" in nombre_upper or "LOQ" in nombre_upper or "ZBOOK" in nombre_upper:
                        precio_semanal_base += 55.00  
                    elif "I7" in nombre_upper or "RYZEN 7" in nombre_upper or "PRECISION" in nombre_upper:
                        precio_semanal_base += 25.00   
                    
                    if "24GB" in nombre_upper or "32GB" in nombre_upper or "64GB" in nombre_upper:
                ...
                conn.commit()
                print(f"[DATA-CORE] ✅ Carga masiva exitosa con tarifas indexadas.")
            except Exception as e:
...
<body class="bg-slate-900 flex items-center justify-center min-h-screen py-10 px-4">
    <div class="bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md border border-slate-700">
        ...
        <form action="{{ url_for('registro') }}" method="POST" class="space-y-4 text-sm">
            <div>
                <label class="block text-slate-300 text-xs font-semibold uppercase mb-1">Token de Autorización Corporativa</label>
                <input type="text" name="admin_token" required placeholder="Ingresar clave aprobada" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white text-xs focus:outline-none focus:border-indigo-500">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="tel" name="telefono" required placeholder="Ej: 999999999" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white text-xs focus:outline-none focus:border-indigo-500">
            </div>
            <div>
                <label class="block text-slate-300 text-xs font-semibold uppercase mb-1">Correo Electrónico de Verificación</label>
                <input type="email" name="email" required placeholder="Ej: correo@servidor.com" class="w-full px-3 py-2 bg-slate-900 border border-slate-700 rounded-lg text-white text-xs focus:outline-none focus:border-indigo-500">
            </div>
            <div>
            ...
            <button type="submit" class="w-full py-2.5 bg-gradient-to-r from-indigo-600 to-blue-600 text-white font-bold rounded-lg shadow-md hover:opacity-90 transition-all">Solicitar Validación de Cuenta</button>
        </form>
...
<body class="bg-slate-900 flex items-center justify-center h-screen px-4">
    <div class="bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-sm border border-slate-700 text-center">
        <h1 class="text-xl font-bold text-white mb-2">Introduce el Código</h1>
        <p class="text-slate-400 text-xs mb-6">Enviamos un token numérico de 6 dígitos a tu bandeja de correo.</p>
        {% with messages = get_flashed_messages() %}
        ...
        <form action="{{ url_for('verificar_codigo') }}" method="POST" class="space-y-4">
            <input type="text" name="codigo_ingresado" maxlength="6" required placeholder="000000" class="w-full tracking-[0.5em] text-center px-4 py-3 bg-slate-900 border border-slate-700 rounded-xl text-lg font-bold text-white focus:outline-none focus:border-indigo-500">
            <button type="submit" class="w-full py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm rounded-lg shadow-lg transition-all">Verificar e Instalar Cuenta</button>
        </form>
        <div class="mt-6 text-center border-t border-slate-700/50 pt-3">
            <a href="{{ url_for('cancelar_registro') }}" class="text-xs text-rose-400 underline">Cancelar Proceso</a>
        </div>
    </div>
</body>
</html>
...
<body class="bg-slate-900 flex items-center justify-center h-screen px-4">
    <div class="bg-slate-800 p-6 sm:p-8 rounded-xl shadow-2xl w-full max-w-md border border-slate-700 text-center space-y-6">
        ...
        <div>
            <h1 class="text-xl font-bold text-white">¡Operador Activado!</h1>
...
<body class="bg-slate-950 text-slate-100 min-h-screen font-sans">
    ...
    // LÓGICA DE OCULTAMIENTO INTELIGENTE Y SELECCIÓN RÁPIDA
    ...
    // Lógica para el cambio manual desde la propia lista desplegable
    ...
    // CÁLCULO DE PRECIO AUTOMÁTICO BASADO EN SEMANAS Y CANTIDAD
    function recalcularPrecio() {
        ...
        let semanasRequeridas = 1; // 1 semana mínima base
        ...
        if(fechaInicio && fechaFin) {
            ...
            if(diferenciaDias > 0) {
                // Redondeamos hacia arriba (ej: 8 dias -> 2 semanas)
                semanasRequeridas = Math.ceil(diferenciaDias / 7);
            } else if (diferenciaDias < 0) {
                // Si la fecha de fin es anterior por error, cobramos la semana mínima y no negativo.
                semanasRequeridas = 1; 
            }
        }
...
def registro():
    if request.method == 'POST':
        ...
        if admin_token not in CREDENCIALES_VALIDAS:
            flash("Token de autorización inválido o fuera de rango.")
        ...
        session['temp_registro'] = {
            ...
            'password': password, 'codigo': codigo_verificacion
        }
...
def verificar_codigo():
    ...
    if request.method == 'POST':
        codigo_ingresado = request.form['codigo_ingresado']
        datos_temp = session['temp_registro']
        if codigo_ingresado == datos_temp['codigo']:
            conn = sqlite3.connect(DATABASE)
            cursor = conn.cursor()
            ...
            flash("Código de autenticación incorrecto.")
...
def procesar_salida():
    ...
    # Interceptor Backend: Captura el YYYY-MM-DD del calendario HTML5 y lo inyecta como DD/MM/YYYY
    try:
